...
def callback(tracks, tracks_corrected, gt):
    ...
    if(len(tracks.tracks) > 1):
        ...
        for i in range(len(tracks.tracks)):
            gt_pos = gt.tracks[0].pos
            track_pos = tracks.tracks[i].pos
            diff_x = abs(gt_pos.x - track_pos.x)
            diff_y = abs(gt_pos.y - track_pos.y)
            dist = diff_x**2+diff_y**2
            if(dist < min_dist):
                id = i
                min_dist = dist
    
    if(math.sqrt(min_dist) > 2): return

    track = tracks.tracks[id]
    track_a = tracks_corrected.tracks[id]
