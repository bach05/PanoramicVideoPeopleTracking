...
class TfPoseEstimator:
    ...
    def draw_humans(npimg, humans, imgcopy=False):
        ...
        for human in humans:
            ...
            # print(box)
            #cv2.rectangle(npimg, (int(box['x']), int(box['y'])), (int(box['w']),int(box['h'])), common.CocoColors[0], thickness=3)
            # draw point
            for i in range(common.CocoPart.Background.value):
                ...
                centers[i] = center
                cv2.circle(
                    overlay, center, 3, common.CocoColors[i], thickness=3, lineType=8, shift=0)
            ...
            for pair_order, pair in enumerate(common.CocoPairsRender):
                ...
                cv2.line(
                    overlay, centers[pair[0]], centers[pair[1]], common.CocoColors[pair_order], 3)

        # apply the overlay
        alpha = 0.4
        cv2.addWeighted(overlay, alpha, npimg, 1 - alpha, 0, npimg)
        
        return npimg

    @staticmethod
    def draw_humans2(npimg, humans, imgcopy=False):
        overlay = np.copy(npimg)*1
        if imgcopy:
            npimg = np.copy(npimg)
        image_h, image_w = npimg.shape[:2]
        ...
        for human in humans:
            ...
            for i in range(common.CocoPart.Background.value):
                ...
                if(i != 1 and i != 10 and i != 13): continue
                body_part = human.body_parts[i]
                center = (int(body_part.x * image_w + 0.5),
                          int(body_part.y * image_h + 0.5))
                centers[i] = center
    ...
    @staticmethod
    def cropped2full_humans(humans, IM_W, IM_H, CROP_RATIO, OFF_X, OFF_Y):
        for human in humans:
            # draw point
            for i in range(common.CocoPart.Background.value):
                if i not in human.body_parts.keys():
                ...
                x1 = body_part.x * IM_W * CROP_RATIO[0]
                y1 = body_part.y * IM_H * CROP_RATIO[1]

                x = x1 + OFF_X
                y = y1 + OFF_Y

                #print("part %d %s\t: %3d,%3d (%1.3f, %1.3f) -> %3d,%3d (%1.3f, %1.3f)" % (i, body_part.get_part_name(), x1, y1, body_part.x ,body_part.y, x, y, x/IM_W, y/IM_H))
    ...
    @staticmethod
    def extended2full_humans(humans, IM_W, ORIGINAL_IM_W):
        for human in humans:
            for i in range(common.CocoPart.Background.value):
                if i not in human.body_parts.keys():
                    continue
                ...
                #body_part.x = math.fmod(body_part.x * ORIGINAL_IM_W / IM_W, 1.0)
                body_part.x = min(0.999999, (body_part.x * IM_W) / ORIGINAL_IM_W)
            
        return humans

    @staticmethod
    def associate_humans(humans1, humans2, IM_W, IM_H, CROP_RATIO, OFF_X, OFF_Y, P_COUNT_THR=12, DIST_THR=0.08, JAC_THR=0.4):
        for human in humans1:
            for i in range(common.CocoPart.Background.value):
                ...
                #print("part %d %s\t: %3d,%3d (%1.3f, %1.3f)" % (i, body_part.get_part_name(), x, y, body_part.x ,body_part.y))
        ...
        #print("\tdetected H2 : %2d"%(len(humans2)))
        #print("\tdetected H1 : %2d"%(len(humans1)))
        ...
        for h2 in humans2:
            ...
            if h2.part_count() < P_COUNT_THR:
                continue

            # Check which joints in parts are in h2
            h2_parts = [i for i in parts if i in h2.body_parts.keys()]
            ...
            for h1 in humans1:
                ...
                # Check which joints in parts are in h2
                h1_parts = [i for i in parts if i in h1.body_parts.keys()]
                ...
                for h2_part in h2_parts:
                    ...
                    # Compute distance
                    h2_p = h2.body_parts[h2_part]
                    h1_p = h1.body_parts[h2_part]

                    dist = math.sqrt((h2_p.x - h1_p.x) **
                                     2 + (h2_p.y - h1_p.y) ** 2)
            ...
            if(len(humans1) and best_jacc > JAC_THR):
                ...
                for i in range(common.CocoPart.Background.value):
                    ...
                    if i in h2.body_parts.keys():
                        #print("Merging : ", i)
                        humans1[max_index].body_parts[i] = h2.body_parts[i]
            else:
                #print("Adding H2")
    ...
    def inference(self, npimg, resize_to_default=True, upsample_size=1.0):
        ...
        #cv2.imshow('peaks',peaks[:,:,0]/np.max(peaks[:,:,0]))
        #cv2.imshow('hm',self.heatMat[:,:,0]/np.max(self.heatMat[:,:,0]))
        #cv2.imshow('pafMat',self.pafMat[:,:,0]/np.max(self.pafMat[:,:,0]))
        logger.debug('inference- peaks=%dx%d' % (peaks.shape[1], peaks.shape[0]))
        t = time.time()
        humans = PoseEstimator.estimate_paf(peaks, self.heatMat, self.pafMat)
        logger.debug('estimate time=%.5f' % (time.time() - t))
        return humans

    def inferenceABC(self, npimg, resize_to_default=True, upsample_size=4.0):
        if npimg is None:
            raise Exception(
                'The image is not valid. Please check your image exists.')
    ...
    def inference2(self, npimgs, OFF_X, OFF_Y, resize_to_default=True, upsample_size=1.0):
        for npimg in npimgs:
            if npimg is None:
                raise Exception(
                    'The image is not valid. Please check your image exists.')
        ...
        for npimg in npimgs:
            logger.debug('inference+ original shape=%dx%d' %
                        (npimgs[0].shape[1], npimgs[0].shape[0]))
        
        img0 = npimgs[0]
        img1 = npimgs[1]
        #cv2.imshow('npimg0', npimgs[0])
        #cv2.imshow('npimg1', npimgs[1])
        ...
        for img in imgs : 
            ...
            #peaks = peaks[0] # to remove
            #self.heatMat = heatMat_up[0] # to remove
            #self.pafMat = pafMat_up[0] # to remove

            pKs.append(peaks[0])
            hMats.append(heatMat_up[0])
            pMats.append(pafMat_up[0])

            logger.debug('inference2- heatMat=%dx%d pafMat=%dx%d' % (
                self.heatMat.shape[1], self.heatMat.shape[0], self.pafMat.shape[1], self.pafMat.shape[0]))

        logger.debug('inference2 time=%.5f' % (time.time() - t))
        # Combine results

        # 1. Resize mats to its original image size
        dim0 = (img0.shape[1], img0.shape[0]) # Big image size
        dim1 = (img1.shape[1], img1.shape[0]) # Cropped image size
        res_hMats0 = cv2.resize(hMats[0], dim0)
        res_hMats1 = cv2.resize(hMats[1], dim1)
        res_pMats0 = cv2.resize(pMats[0], dim0)
        res_pMats1 = cv2.resize(pMats[1], dim1)
        res_pKs0 = cv2.resize(pKs[0], dim0)
        res_pKs1 = cv2.resize(pKs[1], dim1)

        #cv2.imshow('res_hm0',res_hMats0[:,:,0]/np.max(res_hMats0[:,:,0]))
        #cv2.imshow('res_hm1',res_hMats1[:,:,0]/np.max(res_hMats1[:,:,0]))
        

        # 2. Substitute cropped portion of 0 with 1
        res_hMats0[OFF_Y:OFF_Y+res_hMats1.shape[0], OFF_X:OFF_X+res_hMats1.shape[1]] = res_hMats1
        res_pMats0[OFF_Y:OFF_Y+res_pMats1.shape[0], OFF_X:OFF_X+res_pMats1.shape[1]] = res_pMats1
        res_pKs0[OFF_Y:OFF_Y+res_pKs1.shape[0], OFF_X:OFF_X+res_pKs1.shape[1]] = res_pKs1

        # 3. Resize to output size
        peaks = cv2.resize(res_pKs0, (hMats[0].shape[1], hMats[0].shape[0]))
        self.heatMat = cv2.resize(res_hMats0, (hMats[0].shape[1], hMats[0].shape[0]))
        self.pafMat = cv2.resize(res_pMats0, (hMats[0].shape[1], hMats[0].shape[0]))

        #peaks = res_pKs0
        #self.heatMat = res_hMats0
        #self.pafMat = res_pMats0
    ...
    def inference3(self, npimg, resize_to_default=True, upsample_size=1.0):
        ...
        img0 = npimg[:,0:npimg.shape[1]/2]
        img1 = npimg[:,npimg.shape[1]/2:-1]
        ...
        for npimg in imgs : 
            ...
            print('inference3- heatMat=%dx%d pafMat=%dx%d' % (
                heatMat_up[0].shape[1], heatMat_up[0].shape[0], pafMat_up[0].shape[1], pafMat_up[0].shape[0]))
            
            logger.debug('inference3- peaks=%dx%d' % (peaks[0].shape[1], peaks[0].shape[0]))

        logger.debug('inference2 time=%.5f' % (time.time() - t))
        # Combine results

        #pKs[0][:,:,0] = pKs[0][:,:,0]/np.max(pKs[0][:,:,0])*np.max(pKs[1][:,:,0])

        #hMats[0][:,:,0] = hMats[0][:,:,0]/np.max(hMats[0][:,:,0])*np.max(hMats[1][:,:,0])

        #pMats[0][:,:,0] = pMats[0][:,:,0]/np.max(pMats[0][:,:,0])*np.max(pMats[1][:,:,0])

        peaks = np.concatenate((pKs[0], pKs[1]), axis=1)
        self.heatMat = np.concatenate((hMats[0], hMats[1]), axis=1)
        self.pafMat = np.concatenate((pMats[0], pMats[1]), axis=1)

        cv2.imshow('peaks',peaks[:,:,0]/np.max(peaks[:,:,0]))
        cv2.imshow('hm',self.heatMat[:,:,0]/np.max(self.heatMat[:,:,0]))
        cv2.imshow('pafMat',self.pafMat[:,:,0]/np.max(self.pafMat[:,:,0]))

        logger.debug('inference3- peaks=%dx%d' % (peaks.shape[1], peaks.shape[0]))
