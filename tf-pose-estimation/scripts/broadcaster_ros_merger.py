...
def callback(p1_msg, p2_msg):

    global assoc_index_thr
    global part_num_thr

    t1 = time.time()
    humans1 = msg_to_humans(p1_msg)
    humans2 = msg_to_humans(p2_msg)
    rospy.loginfo('msg_to_humans time=%.5f' % (time.time() - t1))
    t = time.time()
    if(p2_msg.crop_x < 9999):
        #crop_ratio = (p2_msg.crop_w / p1_msg.image_w,                    p2_msg.crop_h / p1_msg.image_h)

        humans = associate_humans(humans1, humans2, p1_msg.image_w, p1_msg.image_h, part_num_thr, assoc_index_thr)
...
if __name__ == '__main__':
    ...
    # topics params
    # rospy.get_param('~pose', '/pose_estimator/pose')
    pose1_topic = rospy.get_param('~pose1', '/pose1_estimator/pose1')
    # rospy.get_param('~pose', '/pose_estimator/pose')
    pose2_topic = rospy.get_param('~pose2', '/pose2_estimator/pose2')
    ...
    #ts = message_filters.ApproximateTimeSynchronizer([pose1_sub, pose2_sub], 10, 0.05)
    ts = message_filters.TimeSynchronizer([pose1_sub, pose2_sub], 50)
