...
def callback_image(data):
    ...
    try:
        ...
        rospy.loginfo('A - imgmsg_to_cv2  time=%.5f' % (time.time() - t))
        ...
        imageA = cv_image[int(IM_H*0.2):int(IM_H*0.9),0:tileW+overlap]

        CROP_H = imageA.shape[0]
        CROP_W = imageA.shape[1]
    ...
    try:
        ...
        # Inference on imageA
        humansA = pose_estimator1.inference(imageA, resize_to_default=True, upsample_size=resize_out_ratio)

        # Fix detections
        OFF_X = 0.0
        OFF_Y = int(IM_H*0.2)
        CROP_RATIO = (CROP_W*1.0 / IM_W, 1.0*CROP_H / IM_H)
        humans = TfPoseEstimator.cropped2full_humans(humansA, IM_W, IM_H, CROP_RATIO, OFF_X, OFF_Y)
...
if __name__ == '__main__':
    ...
    pub_topic = rospy.get_param('~pub_topic', '~poseA')
