...
def callback_image(data):
    ...
    try:
        ...
        tileW = IM_W/3
        overlap = int(tileW * 0.03)

        imageB = cv_image[int(IM_H*0.2):int(IM_H*0.9),tileW-overlap:tileW + tileW + overlap]

        CROP_H = imageB.shape[0]
        CROP_W = imageB.shape[1]
    ...
    try:
        ...
        # Inference on imageB
        humansB = pose_estimator1.inference(imageB, resize_to_default=True, upsample_size=resize_out_ratio)

        # Fix detections
        OFF_X = tileW-overlap
        OFF_Y = int(IM_H*0.2)
        CROP_RATIO = (CROP_W*1.0 / IM_W, 1.0*CROP_H / IM_H)
        humans = TfPoseEstimator.cropped2full_humans(humansB, IM_W, IM_H, CROP_RATIO, OFF_X, OFF_Y)
...
if __name__ == '__main__':
    ...
    pub_topic = rospy.get_param('~pub_topic', '~poseB')
