...
public:
  ...
  // associate
  std::vector<Association> associate(const std::vector<Tracker>& trackers /*people*/, const std::vector<Observation>& observations) override {
    if(trackers.empty() || observations.empty()) { // If people or observation are empty there no associations to compute
    ...
    for(int i=0; i<trackers.size(); i++) {
      for(int j=0; j<observations.size(); j++) { // For each person and for each observation
